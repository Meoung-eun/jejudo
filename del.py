...
class MainWindow(QMainWindow, form_class): #화면을 띄우는데 사용되는 Class 선언
    def __init__(self):
        ...
        self.btn_del.clicked.connect(self.pop)
    ...
    def get_checked(self): # 체크박스 클릭할 때마다 행 삭제
        ...
        self.confu = pymysql.connect(host='localhost', user='root', password='0000', db='jejudo',
                               charset='utf8')
        curr = self.confu.cursor()
        print(pop_name)
        # SQL문 실행
        # 검색창에 입력된 'pop_name'을 포함하는 행 삭제하는 기능
        curr.execute(f"DELETE FROM jejudo.jeju_table WHERE Restaurant_name = '{self.checked_item}'")
